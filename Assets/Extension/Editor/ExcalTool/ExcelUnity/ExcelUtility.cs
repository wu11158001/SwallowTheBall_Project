...
public class ExcelUtility
{
	...
	{
		FileStream mStream = File.Open (excelFile, FileMode.Open, FileAccess.Read, FileShare.Read);
	...
	/// <param name="encoding">表头行数</param>
	public void ConvertToJson (string JsonPath, Encoding encoding)
	{
		...
		for (int i = 1; i < rowCount; i++) {
			...
			string rowName = mSheet.Rows[i][0].ToString();
			for (int j = 0; j < colCount; j++) {
				//读取第1行数据作为表头字段
				string field = mSheet.Rows [0] [j].ToString ();
                if (field == "")
                ...
                if (field == "in")
                {
                    row[field] = HindiTranslator.Translate_Hindi(mSheet.Rows[i][j].ToString());
				}
                else
                {
